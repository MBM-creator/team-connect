...
function fromAddress(): string {
  ...
  if (!v) {
    throw new Error('EMAIL_FROM is not set (example: Team Connect <reports@yourdomain.com>)');
  }
  return v;
}
...
function subjectFor(emailActionType: string): string {
  switch (emailActionType) {
    case 'recovery':
      return 'Reset your Team Connect password';
    case 'invite':
      return 'You have been invited to Team Connect';
    case 'signup':
      return 'Confirm your Team Connect email';
    case 'email_change':
      return 'Confirm your email change';
    default:
      return 'Your Team Connect sign-in link';
  }
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const appName = Deno.env.get('APP_NAME')?.trim() || 'Team Connect';
