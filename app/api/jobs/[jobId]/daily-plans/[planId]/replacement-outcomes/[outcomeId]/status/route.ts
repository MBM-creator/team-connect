...
export async function POST(
  ...
  if (!isValidUuid(planId) || !isValidUuid(outcomeId)) {
    return dailyPlanJsonError('Replacement outcome not found', 404, requestId);
  }
  ...
  if (outcomeError) {
    ...
    logDailyPlanMutationFailure({
      ...
      planId,
      userId: staffAuth.staff.id,
      errorType: err.code ?? 'DP_REPLACEMENT',
      message: 'Failed to load replacement outcome',
    });
    return dailyPlanServerError(
      requestId,
      err.code ?? 'DP_REPLACEMENT',
      'Failed to load replacement outcome'
    );
  }
  if (!outcome) {
    return dailyPlanJsonError('Replacement outcome not found on this plan', 404, requestId);
  }
