...
export default function SignOffQaSectionPage() {
  ...
  const sectionCode = isSignoffSectionCode(sectionCodeRaw) ? (sectionCodeRaw as SignoffSectionCode) : null;
  const def = sectionCode ? getSignoffSectionDefinition(sectionCode) : null;
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setPhotoFiles({});
      setPhotoPreviews({});
      setTimeout(() => router.push(`/t/${orgSlug}/jobs/${jobId}/qa/sign-off/${runId}`), 1200);
    } catch {
  ...
  if (!sectionCode || !def) {
    return <div className="min-h-screen bg-gray-50 py-8 px-4"><div className="max-w-xl mx-auto text-red-800">Unknown sign-off section.</div></div>;
  }
